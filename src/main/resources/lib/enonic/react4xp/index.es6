...
const bodyHasContainer = (body, react4xpId) => {
  const react4xpPattern = new RegExp(
    "<[^>]+\\s+id\\s*=\\s*[\"']" + react4xpId + "[\"']",
    "i"
  );
...
const buildContainer = (react4xpId, content) =>
  `<div id="${react4xpId}">${content || ""}</div>`;
...
class React4xp {
  ...
  constructor(initParam) {
    ...
    if (typeof initParam === "object") {
      if (!initParam || !initParam.descriptor || !initParam.type) {
        throw Error(
          `Can't initialize Reac4xp component with initParm = ${JSON.stringify(
            initParam
          )}. Doesn't seem to be a valid XP component object - missing type or descriptor?`
        );
      }
      this.component = initParam;
      const compName = this.component.descriptor.split(":")[1];
      this.jsxPath = `site/${
        BASE_PATHS[this.component.type]
      }/${compName}/${compName}`;
      this.react4xpId = `${BASE_PATHS[this.component.type]}_${compName}_${
        this.component.path
      }`.replace(/\//g, "_");
      ...
      if (this.jsxPath === "") {
        throw Error(
          `Can't initialize Reac4xp component with initParm = ${JSON.stringify(
            initParam
          )}. XP component object or jsxPath string only, please.`
        );
      }
    } else {
      throw Error(
  ...
  static _buildFromParams = params => {
    ...
    if (!component && !jsxPath) {
      throw Error(
        "Can't render React4xp for client: need 'jsxPath' or 'component' parameter"
      );
    }
  ...
  checkIdLock() {
    if (this.react4xpIdLocked) {
      throw Error(
        "This component has already been used to generate a body or pageContributions.es6. " +
  ...
  ensureAndLockBeforeRendering() {
    ...
    if (!this.jsxPath) {
      throw Error(
        "Target path for JSX component, jsxPath, has not been set. Add an absolute path (or an XP component from which to derive it) in the React4XP constructor or with the setters."
      );
    }
    if (!this.react4xpId) {
  ...
  uniqueId() {
    return this.setId(
      (this.react4xpId || "") + "_" + Math.floor(Math.random() * 99999999)
    );
  }
  ...
  setJsxPath(jsxPath) {
    ...
    if (
      ...
      throw Error(
        `React4xp.setJsxFileName: invalid jsxPath (${JSON.stringify(
          jsxPath
        )}). This is a NAME, not a relative path, so it can't be missing/empty, or contain '..', '//', '/./' or start with '.' or '/'.${
          this.component ? ` Component: ${JSON.stringify(this.component)}` : ""
        }`
      );
    }
    ...
    jsxPath =
      jsxPath.endsWith(".jsx") || jsxPath.endsWith(".es6")
        ? jsxPath.slice(0, -4)
        : jsxPath.endsWith(".js")
        ? jsxPath.slice(0, -3)
        : jsxPath;
  ...
  renderTargetContainer(body, content) {
    ...
    if (!bodyHasContainer(body, this.react4xpId)) {
      return HTMLinserter.insertAtEndOfRoot(
        body,
  ...
  renderPageContributions = params => {
    ...
    return getAndMergePageContributions(this.jsxPath, pageContributions, {
      bodyEnd: [
        ...
        `<script defer>${LIBRARY_NAME}.CLIENT.${command}(${LIBRARY_NAME}['${
          this.jsxPath
